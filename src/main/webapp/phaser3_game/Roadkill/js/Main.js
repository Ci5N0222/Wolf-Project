class Main extends Phaser.Scene {
	...
	create() {
		...
		this.player.setScale(1, 1); // 크기 조정
		...
		// 충돌 카운터 텍스트 추가
		this.collisionText = this.add
			.text(10, 10, "[내 목숨 10] 충돌 : 0", {
				fontSize: "30px",
		...
		this.timerText = this.add
			.text(this.cameras.main.width - 10, 5, "0", {
				fontSize: "40px",
				fontFamily: "April16th-Life",
			})
			.setOrigin(1, 0);
		...
		this.physics.add.collider(this.boxes, this.player, (box, player) => {
			...
			if (currentTime - this.lastCollisionTime > 1000) {
				...
				this.collisionText.setText(
					"[내 목숨 " + this.log + "] 충돌 : " + this.playerCollisionCount
				); // 충돌 카운터 텍스트 업데이트
				this.lastCollisionTime = currentTime; // 마지막 충돌 시간 업데이트
				console.log("Game over triggered, score:", this.timerText);
				//목숨 10개 모두 감소되면 GameOver
				// Main 씬에서 GameOver 씬으로 데이터를 전달하여 전환
				if (this.playerCollisionCount >= 10) {
					this.scene.start("GameOver", { score: this.timerText.text });
				}
			}
		});

		// rect = this.add.rec(400, 300, 50, 50, 0xff0000);
